...
def main():
    ...
    with grpc.insecure_channel(f"localhost:50060") as channel:
        ...
        # write_response = stub.Write(book_storage_pb2.WriteRequest(key="2", value=new_book))
        # print(f"Write Response: {write_response.success}")

        # Read the book back from the database
        read_response = stub.Read(book_storage_pb2.ReadRequest(key="3"))
